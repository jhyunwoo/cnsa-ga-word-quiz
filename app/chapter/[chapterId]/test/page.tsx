...
export default function Practice({
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    if (count === 0) {
      ...
      setAnswer(result);
      setResult((prev) => [...prev, result.result]);
    } else {
  ...
  function getAccuracy(resultData: boolean[]) {
    ...
    return (accuracy / resultData.length) * 100;
  }

  useEffect(() => {
    async function update() {
      if (count === quiz.length && count !== 0) {
        const updateResult = await fetch("/api/test", {
          method: "POST",
          body: JSON.stringify({
            chapterId: params.chapterId,
            result: result,
        ...
        const reponse = await updateResult.json();
        console.log(reponse);
      }
    }
    update();
  }, [count, params.chapterId, quiz.length, result]);
          ...
          {quiz.length > 0 && count < quiz.length ? (
            ...
            >
              {quiz[count].type === "word" ? (
                <div className="w-full p-4 rounded-md bg-white shadow-md md:max-w-md md:mx-auto">
                  <input
              ...
              <button
                ...
                } w-full bg-indigo-400 hover:bg-indigo-500 text-white font-semibold rounded-md my-4 p-2 md:max-w-md md:m-auto md:mt-4`}
              >
                정답 확인
              </button>
            </form>
          ...
          <div className="w-full bg-white rounded-lg shadow-lg p-4 flex justify-center items-center md:max-w-md">
            <div className="flex justify-center items-center flex-col w-full">
              <div className="flex justify-center items-center flex-col">
                <div className="text-2xl font-bold py-4">테스트 완료</div>
                <div className="font-semibold pb-4">
                  정확도 : {getAccuracy(result).toFixed(2)}%
                </div>
              </div>
              <Link
